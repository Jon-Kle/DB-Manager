...
import pymysql
import requests
import json  # APIs and database
import cmd
import readline  # Command line
...
class Database:
 ...
 VALUES ('0000-01-01 00:00:00', '26.9', '1014.7', '39', '1.60934', 'SO', '0.0', '2.2');")
                self.cursor.execute(
                    "DELETE FROM `weatherdata` WHERE entryDate = '0000-01-01 00:00:00';")
                self.con.commit()
                return True, None
            except pymysql.Error as e:
                return None, DBWritingError(e)
...
class CLI(cmd.Cmd):
    ...
    def do_debug(self, arg):
        ...
        if arg == '':
            ...
            '''Checks if API1 is working correctly'''
            em = "There was a problem by pinging the Api:"
            try:
                Api1().get_values()
            except ApiConnectionError:
                em += "\n> ApiConnectionError"
...
if __name__ == '__main__':
    ...
    # dl = Download()
    # dl.load('Wetterstation_R.-Steiner-Schule_Ismaning_1-3-22_00-00_1_Day_1647772733_v2.csv')
